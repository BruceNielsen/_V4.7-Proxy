...
namespace PF.PrintLayer
{
    public class Invoice_Work_Orders
    {
        ...
        public static int Print(int Invoice_Id, int int_Trader_Id, string Invoice_Date, string Order_Num, string str_delivery_name, string str_save_filename, decimal dec_GST, string str_type)
        {
            ...
            PF.PrintLayer.Word.ReplaceText("CustomerNum", Convert.ToString(int_Trader_Id));  //Trader_Id
            ...
            ds_Get_Info = PF.Data.AccessLayer.PF_Work_Order.Get_Invoiced(Invoice_Id);
            ...
            DataSet ds = null;
            DataRow dr;
            ds_Get_Info = PF.Data.AccessLayer.PF_A_Invoice_Details.Get_Info_Translated_WO(Invoice_Id);
            for (int i = 0; i < ds_Get_Info.Tables[0].Rows.Count; i++)
            {
                ...
                switch (dr_Get_Info["Code"].ToString().ToUpper().Substring(0, dr_Get_Info["Code"].ToString().IndexOf('-')))
                {
                    ...
                    default:
                        ds = PF.Data.AccessLayer.CM_Bins.Get_NET_Bin_Weight_by_Work_Order(int_wo_id[i]);
                ...
                PF.PrintLayer.Word.ReplaceText("Description" + Convert.ToString(ip), str_desc);
                PF.PrintLayer.Word.ReplaceText("Price" + Convert.ToString(ip), Convert.ToString(Math.Round(Convert.ToDecimal(dr_Get_Info["Value"].ToString()), 2)));
                PF.PrintLayer.Word.ReplaceText("Quantity" + Convert.ToString(ip), str_Quantity);
            ...
            return_code = PF.PrintLayer.General.Print(str_type, str_save_filename, str_Invoice_Date_yyyymmdd);
